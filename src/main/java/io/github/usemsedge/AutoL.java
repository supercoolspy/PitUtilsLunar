...
public class AutoL {
    ...
    static void checkIfSayL(String msg) {
        if (onBountyClaimed) {
            if (msg.startsWith("BOUNTY CLAIMED!") && msg.contains(Minecraft.getMinecraft().thePlayer.getName()) &&
        ...
        if (onBan) {
            if (msg.startsWith("A player has been removed") && !msg.contains(":") && msg.contains("!")) {
                sayL();
            }
        }

        if (onPermList) {
            if (msg.startsWith("KILL!") && msg.contains("[") && msg.contains("]") && !msg.contains(":")
